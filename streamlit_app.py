...
st.set_page_config(page_title="GS Consultoria v25", page_icon="⚡", layout="wide")

# 2. ESTILO DARK MODE COM CORES ELÉTRICAS
st.markdown("""
    <style>
...
# 3. BANCO DE DADOS PROTEGIDO
if 'servicos_db' not in st.session_state:
    st.session_state.servicos_db = {
...
# 4. INICIALIZAÇÃO DE ESTADOS (Evita KeyError)
for key, val in [('logado', False), ('status', 'mural'), ('missao', None), ('ganhos', 0.0)]:
...
if st.session_state.status == "mural":
    ...
    categorias = list(st.session_state.servicos_db.keys())
    tabs = st.tabs(categorias)
    
    for i, cat in enumerate(categorias):
        with tabs[i]:
            for idx, (nome, valor, local) in enumerate(st.session_state.servicos_db[cat]):
                st.markdown(f'<div class="card-trabalho"><b>{nome}</b><br>📍 {local}<br><span class="valor">R$ {valor:.2f}</span></div>', unsafe_allow_html=True)
                # Chave única garantida para cada botão
